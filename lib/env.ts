...
const serverEnvSchema = publicEnvSchema.extend({
  ...
  NETLIFY_IMAGE_GENERATION: z.enum(["sync", "background"]).default("sync"),
  CAMPAIGN_CREDENTIAL_ENCRYPTION_KEYS: z.string().min(1).optional(),
  CAMPAIGN_CREDENTIAL_ACTIVE_KEY_ID: z.string().min(1).optional(),
});
...
export function getServerEnv() {
  return parseEnvironment(serverEnvSchema, {
    ...
    CAMPAIGN_CREDENTIAL_ENCRYPTION_KEYS: process.env.CAMPAIGN_CREDENTIAL_ENCRYPTION_KEYS,
    CAMPAIGN_CREDENTIAL_ACTIVE_KEY_ID: process.env.CAMPAIGN_CREDENTIAL_ACTIVE_KEY_ID,
  });
}
