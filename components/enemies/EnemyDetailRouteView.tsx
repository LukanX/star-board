...
export default function EnemyDetailRouteView({ campaignId, initialResult }: { campaignId: string; initialResult: CampaignEnemyResult }) {
  ...
  return <><CampaignArtEditorSlot /><EnemyPublicRecord campaignId={campaignId} enemy={enemy} isGM={isGM} actions={actions} />{error ? <p className="m-0 mt-3 text-[var(--pink)] text-[10px]" role="alert">{error}</p> : null}{editorOpen ? <EnemyEditor key={`${enemy.id}:${reimportPreview?.sourceSnapshot.contentHash ?? "manual"}`} campaignId={campaignId} enemy={enemy} initialImportPreview={reimportPreview} onCancel={() => { setEditorOpen(false); setReimportPreview(null); }} onSaved={(saved) => { setEnemy(saved); setEditorOpen(false); setReimportPreview(null); }} /> : null}</>;
}
