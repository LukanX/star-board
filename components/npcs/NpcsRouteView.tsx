...
export default function NpcsRouteView({ campaignId, role, initialNpcs, initialPlaces }: { campaignId: string; role: "gm" | "player"; initialNpcs: ApiNpc[]; initialPlaces: ApiPlace[] }) {
  ...
  return <PageLayout eyebrow="ARCHIVE // CONTACTS" title="NPCs" description="People worth knowing, watching, or avoiding. Their private context stays behind the GM lock." action={role === "gm" && !editorOpen ? "ADD NPC" : undefined} actionIcon={<CirclePlus size={16} />} onAction={() => setEditorOpen(true)}>
    {editorOpen ? <NpcEditor campaignId={campaignId} places={places} onCancel={() => setEditorOpen(false)} onSaved={(saved) => { const nextNpc = mapApiNpc(saved, npcs.length); setNpcs((current) => [nextNpc, ...current]); setSelectedNpcId(saved.id); setEditorOpen(false); }} /> : npcs.length ? <ArchiveMasterDetail
