...
export default function NpcCard({ npc, faction, selected, onSelect }: { campaignId: string; npc: NpcRecord; faction?: RelatedFactionSummary | null; selected: boolean; onSelect: (npcId: string) => void }) {
  return <button aria-label={`Select ${npc.name}`} aria-controls="archive-preview-panel" aria-pressed={selected} className={`${recordRowClassName} w-full cursor-pointer text-left focus-visible:outline-1 focus-visible:outline-[var(--cyan)] focus-visible:outline-offset-[-1px] ${npcSelectionClassNames[selected ? "selected" : "idle"]}`} onClick={() => onSelect(npc.id)} type="button">
    ...
    <div className={recordMainClassName}><h3 className="m-0 text-[13px] text-[var(--ink)] [overflow-wrap:anywhere]">{npc.name}</h3><p>{npc.description || "No public profile recorded."}</p>{faction ? <span className="block [overflow-wrap:anywhere] text-[var(--cyan)] font-mono text-[8px] tracking-[.08em]">FACTION // {faction.name}</span> : null}</div>
  </button>;
}