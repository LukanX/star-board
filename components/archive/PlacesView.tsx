...
function PlaceDetail({ place, places, isGM, onEdit, onAddChild }: { place: ApiPlace; places: ApiPlace[]; isGM: boolean; onEdit: () => void; onAddChild: () => void }) {
  return <div className="place-detail"><div className="place-detail-heading"><div><p className="eyebrow">{place.kind.toUpperCase()} RECORD</p><h2>{place.name}</h2><p className="place-breadcrumb"><Map size={13} /> {getPlaceBreadcrumb(places, place.id)}</p></div>{isGM ? <div className="record-row-actions"><button className="icon-button" aria-label={`Edit ${place.name}`} onClick={onEdit} title={`Edit ${place.name}`} type="button"><Pencil size={15} /></button><button className="icon-button" aria-label={`Add child under ${place.name}`} onClick={onAddChild} title={`Add child under ${place.name}`} type="button"><Plus size={15} /></button></div> : null}</div><div className="place-detail-body"><div><p className="eyebrow">PUBLIC BRIEF</p><p>{place.description || "No public description recorded yet."}</p></div><div className="markdown-preview"><div className="preview-toolbar"><FileText size={14} /> PLAYER NOTES <span>PLAYER VISIBLE</span></div><p>{place.player_notes_markdown || "No player notes recorded yet."}</p></div>{isGM ? <div className="markdown-preview place-private-preview"><div className="preview-toolbar"><LockKeyhole size={14} /> GM NOTES <span>PRIVATE</span></div><p>{place.gm_notes_markdown || "No private notes recorded yet."}</p></div> : null}</div></div>;
}