...
export default function PlacesView({
  ...
  useCampaignArtEditor(
    editorOpen
      ? {
          ...
          onChange: (path) =>
            setDraft((current) => ({ ...current, artPath: path })),
          onUrlChange: (url) =>
            setDraft((current) => ({ ...current, artUrl: url })),
          onSubjectChange: (subject) =>
            setDraft((current) => ({ ...current, artSubject: subject })),
          onPromptChange: (prompt) =>
            setDraft((current) => ({ ...current, artPrompt: prompt })),
          onProviderChange: (provider) =>
            setDraft((current) => ({ ...current, artProvider: provider })),
        }
      : null,
  );
  ...
  const savePlace = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!campaignId) {
      onAction(
        "Campaign is unavailable. Return to the campaign selector and try again.",
      );
      return;
    }
    ...
    try {
      const response = await fetch(
        `/api/campaigns/${encodeURIComponent(campaignId)}/places${editingPlace ? `/${encodeURIComponent(editingPlace.id)}` : ""}`,
        {
      ...
      onPlacesChange((current) => {
        ...
        return current.map((place) =>
          place.id === editingPlace.id ? savedPlace : place,
        );
      });
      ...
      onAction(
        editingPlace
          ? `${savedPlace.name} updated.`
          : `${savedPlace.name} added to the place archive.`,
      );
  ...
  const deletePlace = async () => {
    if (
      ...
      !editingPlace ||
      !window.confirm(
        `Delete ${editingPlace.name} from the place archive? Children will become root places.`,
      )
    )
      return;
    ...
    try {
      const response = await fetch(
        `/api/campaigns/${encodeURIComponent(campaignId)}/places/${encodeURIComponent(editingPlace.id)}`,
        { method: "DELETE" },
      );
      ...
      onPlacesChange((current) =>
        current
          .filter((place) => place.id !== editingPlace.id)
          .map((place) =>
            place.parent_place_id === editingPlace.id
  ...
  return (
    <>
      ...
      <div className="page-intro flex items-end justify-between gap-6 mb-[29px] max-[760px]:items-start max-[760px]:flex-col max-[760px]:gap-[19px] max-[760px]:mb-[25px]">
        <div>
          <p className="eyebrow eyebrow-bright">ARCHIVE // PLACE ATLAS</p>
          <h1>Places</h1>
          <p className="intro-copy">
            A genre-neutral atlas for worlds, regions, sites, and the spaces
            between them.
          </p>
        </div>
        {isGM ? (
          <button
            ...
            type="button"
          >
            <CirclePlus size={16} /> ADD ROOT PLACE
          </button>
        ) : null}
      </div>
      {editorOpen ? (
        <section className={`${editorPanelClassName} mb-[18px]`} data-editor-panel="true">
          <div className="editor-heading flex items-start justify-between gap-4 mb-[18px]">
            <div>
              <p className="eyebrow">
                {isGM ? "GM PLACE RECORD" : "PLACE RECORD"}
              </p>
          ...
          {assistantOpen ? (
            <AiDraftAssistant
              ...
              entityLabel="place"
              mode={editingPlace ? "refine" : "create"}
              requestFields={{
                ...(draft.parentPlaceId
              ...
              <label className="place-parent-field">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setDraft((current) => ({
                      ...current,
                      parentPlaceId: event.target.value || null,
                    }))
                  }
                >
            ...
            <label>
              ...
              <textarea
                ...
                onChange={(event) =>
                  setDraft((current) => ({
                    ...current,
                    playerNotesMarkdown: event.target.value,
                  }))
                }
              />
            </label>
            <label>
              ...
              <textarea
                ...
                onChange={(event) =>
                  setDraft((current) => ({
                    ...current,
                    gmNotesMarkdown: event.target.value,
                  }))
                }
              />
            </label>
      ...
      {places.length ? (
        <div className="places-layout">
          <section className="panel places-tree-panel">
            <div className="panel-topline flex items-start justify-between border-b border-[var(--line)] px-[21px] pb-4 pt-5">
              ...
              <Map size={17} className="accent-icon-cyan" />
            </div>
            {normalizedSearch ? (
              <div className="place-search-results">
                {visiblePlaces.length ? (
                  visiblePlaces.map((place) => (
                    <button
                      className={`place-search-result ${selectedPlaceId === place.id ? "place-search-result-active" : ""}`}
                      key={place.id}
                    ...
                    >
                      <span>
                        ...
                        <small>{getPlaceBreadcrumb(places, place.id)}</small>
                      </span>
          ...
          <section className="panel place-detail-panel">
            {selectedPlace ? (
              <PlaceDetail
                place={selectedPlace}
                places={places}
                isGM={isGM}
                onEdit={() => openEditor(selectedPlace)}
                onAddChild={() => openEditor(undefined, selectedPlace.id)}
              />
            ) : (
              <EmptyState
                icon={Map}
                title="Select a place."
                message="Choose a record from the atlas tree to inspect its public brief."
              />
            )}
          </section>
        </div>
...
function PlaceArt({
  ...
  const src =
    place.art_url ??
    (place.art_path?.startsWith("http") ? place.art_path : null);

  return (
    <div
      aria-label={`${place.name} artwork`}
      className={`place-art ${className} ${src ? "has-art" : "no-art"}`}
      role={src ? "img" : undefined}
      style={src ? { backgroundImage: `url(${src})` } : undefined}
    >
      {src ? null : <Map size={18} />}
    </div>
  );
}
...
function PlaceTreeNodeView({
  ...
  return (
    <div className="place-tree-branch">
      <div
        className={`place-tree-row ${selectedPlaceId === node.id ? "place-tree-row-active" : ""}`}
      >
        <button
          className="place-tree-select"
          onClick={() => onSelect(node.id)}
          type="button"
        >
          <span
            className="place-tree-chevron"
            onClick={(event) => {
          ...
          <PlaceArt place={node} className="place-tree-art" />
          <span className="place-tree-copy">
        ...
        {isGM ? (
          <button
            className="w-8 h-8 inline-grid place-items-center border border-transparent bg-transparent text-[var(--muted)] cursor-pointer p-0 hover:text-[var(--ink)] hover:border-[var(--line)] hover:bg-[rgba(255,255,255,.035)] place-tree-add"
            aria-label={`Add child place under ${node.name}`}
      ...
      {expanded ? (
        <div className="place-tree-children">
    ...
    <div className="place-detail">
      <div className="place-detail-heading">
        <div>
          ...
          <h2>{place.name}</h2>
          <p className="place-breadcrumb">
            <Map size={13} /> {getPlaceBreadcrumb(places, place.id)}
          </p>
        </div>
        {isGM ? (
          <div className={recordRowActionsClassName}>
            <button
              className="w-8 h-8 inline-grid place-items-center border border-transparent bg-transparent text-[var(--muted)] cursor-pointer p-0 hover:text-[var(--ink)] hover:border-[var(--line)] hover:bg-[rgba(255,255,255,.035)]"
            ...
            <button
              className="w-8 h-8 inline-grid place-items-center border border-transparent bg-transparent text-[var(--muted)] cursor-pointer p-0 hover:text-[var(--ink)] hover:border-[var(--line)] hover:bg-[rgba(255,255,255,.035)]"
              aria-label={`Add child under ${place.name}`}
              onClick={onAddChild}
              title={`Add child under ${place.name}`}
              type="button"
            >
              <Plus size={15} />
            </button>
      ...
      <PlaceArt place={place} className="place-detail-art" />
      <div className="place-detail-body">
        <div>
          <p className="eyebrow">PUBLIC BRIEF</p>
          <p>{place.description || "No public description recorded yet."}</p>
        </div>
        <MarkdownPreview>
          <MarkdownPreviewToolbar>
        ...
        {isGM ? (
          <MarkdownPreview className="place-private-preview">
            <MarkdownPreviewToolbar>
              <LockKeyhole size={14} /> GM NOTES <span>PRIVATE</span>
            </MarkdownPreviewToolbar>
            <p>{place.gm_notes_markdown || "No private notes recorded yet."}</p>
          </MarkdownPreview>
        ) : null}
      </div>
    </div>
