...
type ImageDraft = {
  ...
  subject: string;
  prompt: string;
  image: { base64: string | null; url: string | null };
  provider: "openai";
  model: string;
  createdAt: string;
};
...
export default function AiArtStudio({ campaignId, kind, subject, currentPrompt, onApproved }: AiArtStudioProps) {
  ...
  const generate = async (mode: "create" | "refine") => {
    ...
    try {
      ...
      const result = (await response.json()) as { error?: string; draft?: ImageDraft };

      if (!response.ok || !result.draft) {
        throw new Error(result.error ?? "The art draft could not be generated.");
      }
...
async function toImageFile(draft: ImageDraft) {
  if (draft.image.base64) {
    ...
    return new File([bytes], "generated-art.png", { type: "image/png" });
  }
  ...
  return new File([blob], "generated-art.png", { type: blob.type || "image/png" });
}
