...
import CampaignSidebar, {
  type CampaignSidebarNavGroup,
} from "@/components/campaign-shell/CampaignSidebar";
...
export default function CampaignRouteShell({
  campaignId,
  campaignName,
  displayName,
  isGM,
  ...
  const groups = campaignNavigation.reduce<CampaignSidebarNavGroup[]>(
    (result, item) => {
      ...
      const navItem = {
        ...item,
        icon: icons[item.id],
        href: campaignSectionPath(campaignId, item.id),
      };
  ...
  const activeLabel =
    campaignNavigation.find((item) => item.id === activeSection)?.label ??
    "Overview";
  ...
  return (
    ...
    >
      ...
      />
      <div data-campaign-content-frame className="mx-auto max-w-[1440px] px-[42px] pb-[62px] pt-9 max-[1100px]:px-[27px] max-[760px]:px-4 max-[760px]:pb-[45px] max-[760px]:pt-[27px]">
        {children}
      </div>
    </CampaignShell>
  );
