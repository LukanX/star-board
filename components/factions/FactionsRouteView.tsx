...
export default function FactionsRouteView({ campaignId, role, initialFactions, initialPlaces }: { campaignId: string; role: "gm" | "player"; initialFactions: ApiFaction[]; initialPlaces: ApiPlace[] }) {
  ...
  return <PageLayout eyebrow="ARCHIVE // POWER MAP" title="Factions" description="The groups shaping this campaign. Use them as mission givers and campaign context." action={role === "gm" ? "ADD FACTION" : undefined} actionIcon={<CirclePlus size={16} />} onAction={() => setEditorOpen(true)}>{editorOpen ? <FactionEditor campaignId={campaignId} places={places} onCancel={() => setEditorOpen(false)} onSaved={(saved) => { const nextFaction = mapApiFaction(saved, factions.length); setFactions((current) => [nextFaction, ...current]); setSelectedFactionId(saved.id); setEditorOpen(false); }} /> : null}{factions.length ? <ArchiveMasterDetail
    ...
    selector={<div data-faction-grid="true" className="grid grid-cols-1 gap-[14px] p-[10px] max-[760px]:gap-[9px]">{visibleFactions.length ? visibleFactions.map((faction) => <FactionCard faction={faction} key={faction.id} selected={selectedFactionId === faction.id} onSelect={setSelectedFactionId} />) : <EmptyState icon={Network} title="No matching factions." message="Try a different name, status, or place." />}</div>}
    preview={selectedFaction ? <FactionPreview campaignId={campaignId} faction={selectedFaction} places={places} /> : null}
    emptyPreview={<div data-faction-detail="true" className="min-w-0 p-[21px] max-[760px]:p-[17px]"><Network className="text-[var(--cyan)]" size={24} /><p className={eyebrowClassName}>ROUTE-OWNED POWER FILES</p><h2>Choose a faction from the map.</h2><p>Select a record to inspect its public brief, status, location, and campaign role.</p></div>}
