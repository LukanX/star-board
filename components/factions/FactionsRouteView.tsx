...
export default function FactionsRouteView({ campaignId, role, initialFactions, initialPlaces }: { campaignId: string; role: "gm" | "player"; initialFactions: ApiFaction[]; initialPlaces: ApiPlace[] }) {
  ...
  return <PageLayout eyebrow="ARCHIVE // POWER MAP" title="Factions" description="The groups shaping this campaign. Use them as mission givers and campaign context." action={role === "gm" && !editorOpen ? "ADD FACTION" : undefined} actionIcon={<CirclePlus size={16} />} onAction={() => setEditorOpen(true)}>{editorOpen ? <FactionEditor campaignId={campaignId} places={places} onCancel={() => setEditorOpen(false)} onSaved={(saved) => { const nextFaction = mapApiFaction(saved, factions.length); setFactions((current) => [nextFaction, ...current]); setSelectedFactionId(saved.id); setEditorOpen(false); }} /> : factions.length ? <ArchiveMasterDetail
