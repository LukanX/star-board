...
export default function FactionCard({ faction, memberCount, selected, onSelect }: { faction: FactionRecord; memberCount?: number; selected: boolean; onSelect: (factionId: string) => void }) {
  return <button aria-label={`Select ${faction.name}`} aria-controls="archive-preview-panel" aria-pressed={selected} className={`faction-${faction.color} flex min-h-[92px] w-full items-center gap-[14px] cursor-pointer overflow-hidden border border-[var(--line)] bg-[var(--panel)] p-[14px_16px] text-left focus-visible:outline-1 focus-visible:outline-[var(--cyan)] focus-visible:outline-offset-[-1px] ${factionSelectionClassNames[selected ? "selected" : "idle"]}`} onClick={() => onSelect(faction.id)} type="button"><FactionEmblem faction={faction} iconSize={24} /><div className="min-w-0"><h3 className="m-0 min-w-0 text-[13px] font-[550] text-[var(--ink)] [overflow-wrap:anywhere]">{faction.name}</h3>{memberCount !== undefined ? <p className="m-[6px_0_0] text-[var(--dim)] font-mono text-[8px] tracking-[.08em]">{memberCount} NPC{memberCount === 1 ? "" : "S"}</p> : null}</div></button>;
}