...
3. Apply `supabase/migrations/0001_initial.sql` through `0020_enemy_source_identity_check.sql` in order through the Supabase SQL editor or the linked Supabase CLI.
...
The interface uses a synthwave starship-terminal visual language. Persistence covers job-board voting, campaign notes, characters/NPCs/factions, the arbitrary-depth Places archive, the campaign-scoped Enemies archive, one primary Place link on NPCs/factions/jobs/episodes, episode promotion, image uploads, and review-before-save AI text, image, and source-import drafts.
